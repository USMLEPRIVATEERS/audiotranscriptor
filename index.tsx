  ...
  private async handleFileUpload(event: Event): Promise<void> {
    ...
    const validFiles = files.filter((file) => {
      if (file.size > maxSizeInMB * 1024 * 1024) {
        console.warn(
          `File "${file.name}" is too large (>${maxSizeInMB}MB) and will be skipped.`,
        );
        return false;
      }
      return true;
    ...
    if (validFiles.length === 0) {
      this.recordingStatus.textContent = `Selected file(s) were too large (max ${maxSizeInMB}MB).`;
      input.value = '';
      return;
    }
    ...
    this.recordingStatus.textContent = `Processing ${fileCount} file${
      fileCount > 1 ? 's' : ''
    }...`;
    ...
    try {
      for (let i = 0; i < validFiles.length; i++) {
        ...
        this.recordingStatus.textContent = `Transcribing "${file.name}" (${
          i + 1
        }/${fileCount})...`;
      ...
      if (allTranscriptions.length > 0) {
        ...
        this.recordingStatus.textContent =
          'No transcriptions were generated from the selected files.';
      }
    } catch (error) {
